...
# Streamlit 페이지 설정
st.title("H-R Diagram with Future Stellar Evolution Path (10 Stars)")
st.write("10개의 별의 질량을 입력하여 H-R 다이어그램 상의 현재 위치와 미래 진화 경로를 시각화합니다.")
...
3. H-R 다이어그램에 각 별의 현재 위치(다양한 색상 점)와 미래 진화 경로(녹색 계열 곡선, 화살표)가 표시됩니다.

### 참고
- 광도와 온도는 질량에 따라 보정된 관계(L ∝ M^2.3~4.0, T ∝ M^0.505)를 사용해 추정됩니다.
- 주계열선은 질량 범위(0.1~100 M☉)에 맞춰 현실적으로 조정되었습니다.
- 진화 경로는 현재 주계열성에서 미래 단계(적색거성, 백색왜성, 초신성 등)로만 계산되며, 베지어 곡선으로 부드럽게 표현되며, 화살표로 방향을 나타냅니다.
